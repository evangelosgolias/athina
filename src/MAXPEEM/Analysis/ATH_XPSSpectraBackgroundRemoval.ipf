...
// It is adopted here with minor modifications to adjust to the ATH naming conversions, database and workflow.  
...
// EG: Moved the launcher to MAXPEEM>Analysis>XPS. See ATH_CustomIgorMenus.ipf in "MAXPEEM" menu.
// There you can find another was to launch the panel using the TracePopup menu (see ATH_LaunchRemoveXPSBackground())
...
Function BackgroundSubtractGUI() //BackgroundSubtractGUI(inwave)
	...
	WAVE inwave = TraceNameToWaveRef(winNameStr, graphNameStr) // /Z for debug needed
...
//	String workF = "BG"+ReplaceString("InelasticBackDisplay",gName,"")+"_"+CleanupName(NameOfWave(inwave),0)[0,25]
	//EG: Change to our data folder structure under Packages:ATH_DataFolder
...
static Function/DF getWorkFolder(gName)															// grab current working folder from graph info
	...
	if (strlen(folder))
	//EG: : Change to our data folder structure under Packages:ATH_DataFolder
		return root:Packages:ATH_DataFolder:$(kWorkingDir):$(folder)
	else	
		return root:Packages:ATH_DataFolder:$(kWorkingDir) 												// backwards compatibility with older sessions
	endif
End
//################################################################################################
Function BackRem_WindowHook(s)																	// main window hook
	...
	//EG: Change to our data folder structure under Packages:ATH_DataFolder
	DFREF pkg = root:Packages:ATH_DataFolder:
	Switch (s.EventCode)
