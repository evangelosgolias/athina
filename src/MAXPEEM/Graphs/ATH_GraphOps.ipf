...
Function ATH_DuplicateWaveAndDisplayOfTopImage()
    ...
    if(!V_flag && WaveDims(wRef) == 3) // If there is no 3d axis, there should be a reason.
    	...
    	ATH_DisplayImage(cdfr:$waveNameStr)
    endif
...
Function ATH_TextAnnotationOnTopGraph(string text, [variable fSize, string color])
